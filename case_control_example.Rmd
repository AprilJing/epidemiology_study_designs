...
# 2x2 table
xtabs(~ Dis + Exp, case_control_data)

# regression model (unadjusted)
casecont_est <- tidy(glm(Dis~ Exp , 
  data = c_sex_df, family = "binomial"(link="logit"))) %>% 
...
round(casecont_est,2)

```

Our crude odds ratio for this case-control sample suggests no association between exposure and disease (OR:0.96, 95%CI: 0.78-1.10). However, we know sex is a confounding variable based on our simulation.

```{r case control adj model}
# 2x2 table, stratified by sex
xtabs(~ Dis + Exp + Sex, case_control_data)
...
When we adjust for sex, we arrive at the appropriate estimate between exposure and disease. We'd conclude there is a significant association between exposure and disease, adjusting for sex (OR:2.06, 95%CI:1.71-2.48).
...
```{r 1 to 1 matched case-control, message = F}

# sloppy code to match. One day I may make a formula for 1x1 matching

# create all combinations of matching variables, then assign each stratum a number
strata_df<-expand.grid(Sex=unique(c_sex_df$Sex))
strata_df$stratnum <- 1:dim(strata_df)[1]

# merge strata numbers back to original DF
case_control_data <- merge(c_sex_df, strata_df, by=c("Sex"), all.x=TRUE)

# random subset of cases:
set.seed(123)
cases <- case_control_data %>% 
  filter(Dis == 1) %>% 
  sample_n(100)

# table to get the number of cases per stratum
case_stratum_list <- data.frame(table(cases$stratnum))

#create a df of only controls
controls <- case_control_data %>% 
  filter(Dis == 0)

#ratio of controls per case
controls_per_case <- 4

# create final sample, starting with the cases, and the controls will be added as 
# controls are selected for each stratum

casecontrol_sample <- cases
#loop (ugh) to sample the cases per stratum
for (i in 1:dim(case_stratum_list)[1]){
  #Long line-- as i loops through the strata, 
    # 1 subset the controls to the current stratum
    # 2 select a random sample of controls from that stratum
    #   (number of controls == number of cases in stratum * ratio for controls per case)
    # 3 subset the control DF to only selected random sample
    # 4 rbind to final DF
  casecontrol_sample <- rbind(casecontrol_sample, 
                        controls[sample( which(controls$stratnum==case_stratum_list[i,1]), 
                                         controls_per_case * case_stratum_list[i,2]),])
}
  
with(casecontrol_sample, table(Dis, stratnum))


# Let's check the distribution of exposure by sex in our cases
xtabs(~Exp + Sex + Dis, casecontrol_sample)
xtabs(~Dis + Sex, matched_case_control)

xtabs(~Dis + Exp + stratnum, casecontrol_sample)

library(survival)
model<-clogit(Dis ~ Exp + strata(stratnum), casecontrol_sample)

summary(glm(Dis ~ Exp, casecontrol_sample, family = "binomial"))
summary(model)
confint(model)
```


```{r confounding age and sex, message=F}
# this chunk of code uses the simcausal package
...
```{r pop confound sim stats, message=F}
