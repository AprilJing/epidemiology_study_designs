...
for(i in 1:length(n_case_control)){
  ...
  or_est <- quantile(or_vector, c(0.025, 0.5, 0.975))
...
ggplot(bias_df, aes(x=samp_n)) +
  geom_ribbon(aes(ymin=perc_bias_lower, ymax=perc_bias_upper), alpha=0.5,
              fill = "blue") +
  geom_line(aes(y = perc_bias_median), color = "blue") +
  ggtitle("Percent Bias vs. Sample Size") +
  ylab(expression("Percent Bias")) +
...
This plot shows the range of percent bias [(True Odds Ratio - Case-Control Odds Ratio)/ True Odds Ratio] improves as sample size increases. The closer to 0, the less bias.

Notice the median estimate (blue line running right across 0) is perfectly at 0, suggesting no bias. This is a good example of why it's important to replicate the study question over and over. Over 1000 replicates, the median values of all those replicates has no bias. However, you can see the 95% bounds based on sample size narrow as sample size increases. I'd say that around 100 cases and controls, sample size may stablize a bit.
